...
class AccountController extends Controller
{
    public function getIndex()
    {
        ...
        if (Auth::user()->user_type == UserTypes::$STUDENT){
            $user_detail = Student::where('user_id',Auth::user()->id);
        }
        if (Auth::user()->user_type == UserTypes::$TEACHER){
            $user_detail = Teacher::where('user_id',Auth::user()->id);
        }
        ...
        public function UserProfileUpdate(Request $request)
    {
        ...
        if ($emailExists==true) {
            ...
            DB::table('users')->where('id', $user_id)->update(['email' => $request->email , 'name' => $request->name]);
            $updateData = [
